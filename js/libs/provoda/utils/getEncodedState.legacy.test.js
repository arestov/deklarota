...
describe('modern addr used as legacy addr', () => {
  ...
  it.each([
    ...
    ['^hostnameToSearch'],
    ['^query'],
    ['@one:$meta$rels$itemsList$length:results'],
    ['@one:isLoading:recipeNavigation'],
    ['@one:attemptedLoad:recipeNavigation'],
    ['@one:$meta$rels$freshItemsList$loading:results'],
    ['@one:combined:recipeNavigation'],
    ...
    ['^isActive'],
    ['^nam'],
  ])('should work for "%s"', legacyAddr => {
    const legacyParsed = getParsedState(legacyAddr)
    const legacyParsedFromModern = modernAsLegacyParsed(legacyAddrToModern(legacyAddr))
    expect(clean(legacyParsed)).toEqual(clean(legacyParsedFromModern))
  })
})

function legacyAddrToModern(legacyAddr) {
  return asString(fromLegacy(legacyAddr))
}

function clean(obj) {
  // we don't compare full_name, zip_name, nil_allowed (so don't use `&` prefix in test fixtures)
  return deepCopyWithout(obj, ['full_name', 'zip_name', 'nil_allowed'])
}

function deepCopyWithout(src, fields) {
